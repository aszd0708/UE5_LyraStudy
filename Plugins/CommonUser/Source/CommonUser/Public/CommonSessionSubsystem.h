// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "Subsystems/GameInstanceSubsystem.h"
#include "CommonSessionSubsystem.generated.h"

/*
* UCommonSession_HostSessionRequest를 만들어 CommonSessioSubsystem 을  전달하면, CommonSessionSubsystem에서 MapID와 CmdArgs로 최종적인 CmdArgs를 만들어 전달해준다.
*/
UCLASS(BlueprintType)
class COMMONUSER_API UCommonSession_HostSessionRequest : public UObject
{
	GENERATED_BODY()
public:
	/* MapID -> Text 변환 */
	FString GetMapName() const;

	/* Serveravel에 전달할 최종 URL을 생성한다. */
	FString ConstructTravelURL() const;

	/* 준비할 MapID (맵 경로) */
	UPROPERTY(BlueprintReadWrite, Category = Session)
	FPrimaryAssetId MapID;

	/* 전달할 CmdArgs (Experience 이름을 전달함) -> 곧 UserFacingExperienceDefinition에서 보게 될 것 */
	TMap<FString, FString> ExtraArgs;
};

/**
 * UCommonSessionSubsystem은 LyraStudy에서 사용되야 하기 때문에, Module Export를 해줘야 하고 그러기 때문에 COMMONUSER_API를 추가해줘야 함.
 * - 만약, {ModuleName}_API의 추가는 다른 모듈에서 사용할 경우, 추가해주면 됨.
 */
UCLASS()
class COMMONUSER_API UCommonSessionSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

public:
	UCommonSessionSubsystem() {};

	UFUNCTION(BlueprintCallable, Category = Session)
	void HostSession(APlayerController* HostingPlayer, UCommonSession_HostSessionRequest* Request);

	/*
	* member variables
	*/
	/* PendingTravelURL은 맵의 경로로 생각 */
	FString PendingTravelURL;
};
